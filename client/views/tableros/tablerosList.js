Template.tablerosList.rendered = function(){
  s = Session.get('colSelectedTableros');
  if( s === undefined || s === false || _.isEmpty(s)){
    Session.set('colSelectedTableros',['nombre',]);
  }
}
Template.tablerosList.helpers({
  tableros: function () {
    return Tableros.find().fetch();  
  },
  
  showWarningDeletedTablero: function(){
    ldb = Session.get('lastDeletedTablero');
  ...
  toConfirmDelete: function(tableroId){
    if(Session.get('toConfirmDelete') !== undefined && Session.get('toConfirmDelete') && Session.get('edit_tableroId') == tableroId){
      return "";   
    }else{
      return "hidden";
    }
  ...
  toEnableDelete: function(tableroId){
...
Template.tablerosList.events({
  ...
  "click .colSelect" : function(event){  
    ...
    if( s === undefined || s === false || _.isEmpty(s)){
      Session.set('colSelectedTableross',['name']); 
    }else{
      if(_.contains(s,colClicked)){
        if(s.length > 1){
          Session.set('colSelectedTableros',_.without(s,colClicked));
        }
      }else{
        Session.set('colSelectedTableros',_.union(s,colClicked));
      }
    }
  },
  ...
  'click .rowTable': function (event) {
    ...
    var tableroId = this;
    Router.go("/tablero/"+ tableroId._id);
  },
  
  'click #undoDeletedTablero': function (event) {
    console.log('inside undo');
    lda = Session.get('lastDeletedTablero');
    Tableros.insert(_.omit(lda, '_id'));
    Session.set("lastDeletedTablero",null);
  },
...
Template.tablerosList.helpers({
  ...
  tablerosTable: function () {
    return Tableros;  
  },
  
  tableSettings: function () {
    
        var sel = Session.get('colSelectedTableros');
